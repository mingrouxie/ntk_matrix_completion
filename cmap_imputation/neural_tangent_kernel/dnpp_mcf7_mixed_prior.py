...
if __name__ == '__main__':
    ...
    # Delete 'DNPP' from path
    ind = path_prefix.rfind(technique)
    save_prefix = path_prefix[:ind] + "MixedDNPPNTK" + path_prefix[ind + len(technique):]
    ...
    for train, test in iterator:
        # Complex prior involving DNPP and Only Train Combination
        ...
        try:
            dnpp_encoding = pd.read_pickle(path_prefix + "Predictions.pkl")

        except FileNotFoundError:
            raise AssertionError(f"Please train the DNPP Model at path {path_prefix + 'Predictions.pkl'}")

        dnpp_encoding = dnpp_encoding[dnpp_encoding.index.isin(test_high_data.index)][test_high_data.columns].reindex(test_high_data.index)

        ##### Encoding for examples with high number of elements in train
        embedding_high[:train.shape[0], :] = train.to_numpy()
        embedding_high[train.shape[0]:, :] = dnpp_encoding[:len(test_high_data)]
